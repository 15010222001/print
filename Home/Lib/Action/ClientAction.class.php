...
class ClientAction extends CommonAction {
	...
	//添加处理
	public function clientimport_do() {
	    //验证用户权限
	    ...
	    if ($this->isPost()) {
	        ...
	        $name      = $_FILES['client_info']['tmp_name'];
	        ...
	        for($row=2;$row<=$highestRow;$row++){
	            ...
	            $data['ClientType']    = trim($currentSheet->getCellByColumnAndRow(5, $row)->getValue());
	            $data['ClientLevel']   = trim($currentSheet->getCellByColumnAndRow(6, $row)->getValue());
	            $data['ClientSource']  = trim($currentSheet->getCellByColumnAndRow(7, $row)->getValue());
	        ...
	        foreach ($contents as $k=>$v){
	            ...
	            $cli['ClientType']     = $v['ClientType'];
	            $cli['ClientLevel']    = $v['ClientLevel'];
	            $cli['ClientSource']   = $v['ClientSource'];
	        ...
	        $this->success('导入成功',__APP__.'/Client/clientimport',3);
	    }else {
	        parent::operating(__ACTION__,1,'非法请求');
	        $this->error('非法请求');
	    }
