...
class InventoryAction extends CommonAction {
	public function index() {
		...
		$volist = $user->relation(true)->where($where)->order('id desc')->limit($Page->firstRow.','.$Page->listRows)->select();
		if (count($volist) > 0) {
		    //循环取出对应下拉菜单的数据
		    for($i=0; $i<count($volist); $i++) {
		        for($j=0; $j<count($dlist); $j++) {
		            if ($volist[$i]['warehouse_id'] == $dlist[$j]['ID']) {
		                $volist[$i]['warehouse_name'] = $dlist[$j]['MenuName'];
		            }
		        }
		    }
		    //输出数据
		    ...
		    foreach($volist as $k=>$vo) {
		        $volist[$k]['warehouse_name'] = $vo['warehouse_name'];
		    }
		}
	...
	public function log() {
	    ...
	    $dmenu = M('dmenu');
	    $dlist = $dmenu->order('Sortid asc')->select();
	    $volist = $user->relation(true)->where($where)->order('log_id desc')->limit($Page->firstRow.','.$Page->listRows)->select();
	    if (count($volist) > 0) {
	        //循环取出对应下拉菜单的数据
	        for($i=0; $i<count($volist); $i++) {
	            for($j=0; $j<count($dlist); $j++) {
