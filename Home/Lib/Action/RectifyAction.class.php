<?php
//客户管理类
class RectifyAction extends CommonAction {
	...
	public function indexajax() {
		...
		$rectify = D('Rectify_order');
		import('ORG.Util.Page');						// 导入分页类
		if($keyword!=""){
		    $where['order_sn'] = $keyword;
		}
		$count = $rectify->where($where)->count();			//总记录数
		...
		$volist = $rectify->relation(true)->where($where)->order('order_id desc')->limit($Page->firstRow.','.$Page->listRows)->select();
		$html = '';
		//判断有无数据
		if (count($volist) > 0) {
			//循环取出对应下拉菜单的数据
			for($i=0; $i<count($volist); $i++) {
				for($j=0; $j<count($dlist); $j++) {
					if ($volist[$i]['RectifyType'] == $dlist[$j]['ID']) {
						$volist[$i]['RectifyType'] = $dlist[$j]['MenuName'];
					}
					if ($volist[$i]['RectifyLevel'] == $dlist[$j]['ID']) {
						$volist[$i]['RectifyLevel'] = $dlist[$j]['MenuName'];
					}
					if ($volist[$i]['RectifySource'] == $dlist[$j]['ID']) {
						$volist[$i]['RectifySource'] = $dlist[$j]['MenuName'];
					}
			...
			//输出数据
			$i = 1;
			$rectify_order_status = C('rectify_order_status');
			foreach($volist as $vo) {
				...
				$html .= "
					<tr class='tr ".$tr2."'>
						...
						<td class='tc'>".$rectify_order_status[$vo['order_status']]."</td>
	...
	public function opensharedo() {
		...
		if ($rectify->where("ID = $id")->save(array('OpenShare' => $openshare))) {
			parent::operating(__ACTION__,0,'共享客户编号为：'.$id.'的数据');
			R('Public/errjson',array('ok'));
		}else {
			parent::operating(__ACTION__,1,'共享失败：'.$rectify->getError());
	...
	public function openshareajax() {
		...
		$dmenu = M('dmenu');
		$dlist = $dmenu->order('Sortid asc')->select();
		$volist = $rectify->relation(true)->where($where)->order('FinalTime desc')->limit($Page->firstRow.','.$Page->listRows)->select();
		$html = '';
		//判断有无数据
		if (count($volist) > 0) {
			//循环取出对应下拉菜单的数据
			...
			foreach($volist as $vo) {
				...
				$html .= "
					<tr class='tr ".$tr2."'>
						...
						<td class='tc'>".$vo['RectifyType']."</td>
						<td>".$vo['Industry']."</td>
						<td>".$vo['RectifyLevel']."</td>
						<td class='tc'>".$vo['RectifySource']."</td>
						<td class='tc'>".$vo['FollowUp']."</td>
						<td class='tc'>".$vo['Intent']."</td>
						<td class='tc'><a class='uid' href='".C('TMPL_PARSE_STRING.__APP__')."/Rectify/openshareajax/?uid=".$vo['Uid']."'>".$vo['Username']."</a></td>
	...
	public function openshare_company_detail() {
		...
		//查出相应数据
		$rectify = D('Rectify');
		$result = $rectify->relation(true)->where("ID = $id AND Recycle = 0 AND OpenShare = 1")->find();
		if (!$result) {
				...
				if ($vo['Recycle']==0) {
					...
					$html .= "
						<tr class='tr ".$tr2."'>
							...
							<td class='tc'>".$this->Beautifytime($vo['FinalTime'])."</td>
							<td class='tc fixed_w'><a href='".C('TMPL_PARSE_STRING.__APP__')."/Rectify/openshare_contact_detail?id=".$vo['ID']."' class='edit'>查看详细</a></td>
						</tr>
					";
					$i++;
				}
	...
	//新增客户
	public function rectifyadd() {
		parent::win_userauth(92);
		$dmenu=M('dmenu');
		$volist=$dmenu->where('Sid <> 0')->order('Sortid asc')->select();
		$this->assign('volist',$volist);
		$this->display('rectifyadd');
	}
	//导入客户
	public function rectifyimport() {
	    parent::win_userauth(66);
	    $dmenu=M('dmenu');
	    $volist=$dmenu->where('Sid <> 0')->order('Sortid asc')->select();
	    $this->assign('volist',$volist);
	    $this->display('rectifyimport');
	}
	//添加处理
	public function rectifyadd_do() {
		//验证用户权限
		parent::win_userauth(92);
		if ($this->isPost()) {
			$data=array();
			...
			$data['order_sn']      = D('Base')->create_fast_order_sn('rectify');
			...
			//自动完成验证与新增
			$rectify=D('Rectify_order');
			if ($rectify->create($data)) {
			    $rectify->add();
			    parent::rectify($rectify->getLastInsID(),0,'新增调整单');
				parent::operating(__ACTION__,0,'新增调整单：'.$data['order_sn']);
				$this->success('添加成功',__APP__.'/Rectify/rectifyadd',3);
			}else {
			    echo $rectify->getError();exit;
				parent::operating(__ACTION__,1,'新增失败：'.$rectify->getError());
				$this->error($rectify->getError());
			}
		}else {
	...
	//添加处理
	public function rectifyimport_do() {
	    //验证用户权限
	    ...
	    if ($this->isPost()) {
	        ...
	        $name      = $_FILES['Rectify_info']['tmp_name'];
	        ...
	        for($row=2;$row<=$highestRow;$row++){
	            ...
	            $data['RectifyType']    = trim($currentSheet->getCellByColumnAndRow(5, $row)->getValue());
	            $data['RectifyLevel']   = trim($currentSheet->getCellByColumnAndRow(6, $row)->getValue());
	            $data['RectifySource']  = trim($currentSheet->getCellByColumnAndRow(7, $row)->getValue());
	        ...
	        foreach ($contents as $k=>$v){
	            ...
	            $cli['RectifyType']     = $v['RectifyType'];
	            $cli['RectifyLevel']    = $v['RectifyLevel'];
	            $cli['RectifySource']   = $v['RectifySource'];
	            ...
	            if ($rectify->create($cli)) {
	                //添加联系人
	                $cid = $rectify->order('ID desc')->getField('ID')+1;
	                $cont['Cid'] = $cid++;
	                ...
	                if ($contact->create($cont)) {
	                    $id = $rectify->add();
	                    $cid = $contact->add();
	                    $rectify->where("ID=$id")->Save(array('Cid' => $cid));
	                }else {
	        ...
	        $this->success('导入成功',__APP__.'/Rectify/Rectifyimport',3);
	    }else {
	        parent::operating(__ACTION__,1,'非法请求');
	        $this->error('非法请求');
	    }
	...
	public function rectifyedit() {
		...
		//查出相应数据
		$rectify = D('Rectify_order');
		$result = $rectify->join(" tp_user ON tp_rectify_order.add_user = tp_user.ID ")->where("order_id = $id ")->find();
		if (!$result) {
			parent::operating(__ACTION__,1,'没有找到数据：'.$id);
			$this->content='不存在你要修改的数据，请关闭本窗口';
			exit($this->display('Public:err'));
		}
		$rectify_order_goods = D('Rectify_order_goods');
		$goods_list = $rectify_order_goods->join(' tp_goods ON tp_rectify_order_goods.goods_id = tp_goods.goods_id')->where("tp_rectify_order_goods.order_id = $id ")->field('tp_rectify_order_goods.*,tp_goods.goods_name , tp_rectify_order_goods.price*tp_rectify_order_goods.number AS sum_price')->select();
		//echo "<pre>";print_r($rectify_order_goods->getLastSql());exit;
		$rectify_order_status = C('rectify_order_status');
		$rectify_order_action = D('Rectify_order_action');
		$action_list = $rectify_order_action->where("order_id = $id ")->order('action_id DESC')->select();
		if(!parent::userauth3(100)){
		    $result['is_price'] = 0;
		}else{
		    $result['is_price'] = 1;
		...
		$this->assign('rectify_order_status',$rectify_order_status);
		$this->assign('result',$result);
		$this->assign('rectify_order_status',$rectify_order_status);
		$this->display('rectifyedit');
	}
	//修改客户资料处理
	public function Rectifyedit_do() {
		//验证用户权限
		...
		if ($this->isPost()) {
			...
			$data['warehouse_id']  = I('post.warehouse_id','','htmlspecialchars');
			$data['notes']         = I('post.notes','','htmlspecialchars');
			$rectify = M('Rectify_order');
			if ($rectify->create($data)) {
				$rectify->save();
				parent::operating(__ACTION__,0,'更新调整单：'.$data['order_sn']);
				$this->success('调整单更新成功',__APP__.'/Rectify/Rectifyedit?id='.$data['order_id']);
			}else {
	...
	public function Rectify_indel() {
		...
		if ($this->isAjax()) {
			...
			$map['ID'] = array('in',$id);
			$co['Cid'] = array('in',$id);
			if ($rectify->where($map)->save(array('Recycle' => 1))) {
				...
				if ($vo['Recycle']==0) {
					...
					$html .= "
						<tr class='tr ".$tr2."'>
							<td class='tc'><input type='checkbox' class='delid' value='".$vo['ID']."' /></td>
							<td class='tc'>".$vo['ID']."</td>
							<td><a title='点击只显示此公司的联系人' class='compyname' href='".C('TMPL_PARSE_STRING.__APP__')."/Rectify/contactajax?Cid=".$vo['Cid']."'>".$vo['CompanyName']."</a><img class='add' src='".C('TMPL_PARSE_STRING.__IMAGE__')."/icon.png' alt='".C('TMPL_PARSE_STRING.__APP__')."/With/withadd?Cid=".$vo['Cid']."' border='0' title='新增跟单' /></td>
	...
	public function contactadd() {
		...
		$volist=$dmenu->where('Sid <> 0')->order('Sortid asc')->select();	
		$rectify = M('Rectify');
		$result = $rectify->where("ID = $Cid AND Uid=".$_SESSION['ThinkUser']['ID'])->find();
		$this->assign('volist',$volist);
		$this->assign('result',$result);
	...
	public function contactadd_do() {
		...
		if ($this->isPost()) {
			...
			if ($contact->create($cont)) {
				...
				$this->success('添加成功',__APP__.'/Rectify/contactadd?Cid='.$cont['Cid'],3);
			}else {
	...
	public function goodsedit() {
		...
		$volist=$dmenu->where('Sid <> 0')->order('Sortid asc')->select();
		//查出相应数据
		$contact = D('Rectify_order_goods');
		$result = $contact->join(" tp_goods ON tp_rectify_order_goods.goods_id = tp_goods.goods_id ")->where("order_goods_id = $id")->field("tp_rectify_order_goods.*,tp_goods.goods_name")->find();
		if (!$result) {
			parent::operating(__ACTION__,1,'没有找到数据：'.$id);
			$this->content='不存在你要修改的数据，请关闭本窗口';
			exit($this->display('Public:err'));
	...
	public function goodsedit_do() {
		...
		if ($this->isPost()) {
			...
			//自动完成验证与新增
			$rectify_goods=D('Rectify_order_goods');
			$rectify      =D('Rectify_order');
			$_data = $rectify_goods->where(array('order_goods_id'=>$cont['order_goods_id']))->find();
			if ($rectify_goods->create($cont)) {
				$rectify_goods->save();
				$rectify->updateOrderinfo($_data['order_id']);
				parent::operating(__ACTION__,0,'更新成功：'.$cont['order_goods_id']);
				$this->success('修改成功',__APP__.'/Rectify/goodsedit?id='.$cont['order_goods_id'],3);
			}else {
				parent::operating(__ACTION__,1,'更新失败：'.$rectify_goods->getError());
				$this->error($rectify_goods->getError());
			}
		}else {
			parent::operating(__ACTION__,1,'非法请求');
			$this->error('非法请求');
	...
	public function goods_del() {
		...
		if ($this->isAjax()) {
			...
			if ($id=='' || !is_numeric($id)) {
				...
				$id=intval($id);
				$rectify_goods = M('Rectify_order_goods');
				$where = array('order_goods_id'=>$id);
				$_data = $rectify_goods->where($where)->find();
				if ($rectify_goods->where($where)->delete()) {
				    //echo $rectify->getLastSql();exit;
				    $rectify = D('Rectify_order');
				    $rectify->updateOrderinfo($_data['order_id']);
					parent::operating(__ACTION__,0,'删除成功：'.$id);
					R('Public/errjson',array('ok'));
				}else {
					parent::operating(__ACTION__,1,'删除失败：'.$rectify_goods->getError());
					R('Public/errjson',array($rectify_goods->getError()));
				}
			}
		}else {
			parent::operating(__ACTION__,1,'非法请求');
	...
	public function addgoods() {
		...
		if ($this->isAjax()) {
			...
			foreach($_REQUEST['item'] as $k=>$v){
			    ...
			    $result = $rectify_goods->add($v,array(),true);
			}
			$rectify->updateOrderinfo($_REQUEST['order_id']);
	...
	public function reduction() {
		...
		if ($this->isAjax()) {
			...
			if ($id=='' || !is_numeric($id)) {
				...
				$contact=M('contact');
				$where=array('ID'=>$id);
				if ($rectify->where($where)->getField('ID')) {
					$rectify->where($where)->save(array('Recycle' => 0));
	...
	//批量删除
	public function Rectify_c_indel() {
		//验证用户权限
		parent::userauth(94);
		if ($this->isAjax()) {
			if (!$delid=explode(',',I('post.delid',''))) {
	...
	public function confirm() {
	    ...
	    if ($this->isAjax()) {
	        ...
	        if ($order_id=='' || !is_numeric($order_id)) {
	            ...
	            $data['order_status'] = 1;
	            if ($rectify->where($where)->save($data)) {
	                D('Warehouse_inventory')->rectifyConfirm($order_id);
	                parent::rectify($order_id,1,'确认调整单');
	                parent::operating(__ACTION__,0,'确认成功：'.$id);
	                R('Public/errjson',array('ok'));
	            }else {
	                parent::operating(__ACTION__,1,'确认失败：'.$rectify_goods->getError());
	                R('Public/errjson',array($rectify_goods->getError()));
	            }
	        }
	    }else {
	...
	public function unconfirm() {
	    ...
	    if ($this->isAjax()) {
	        ...
	        if ($order_id=='' || !is_numeric($order_id)) {
	            ...
	            $data['order_status'] = 0;
	            if ($rectify->where($where)->save($data)) {
	                D('Warehouse_inventory')->rectifyunConfirm($order_id);
	                parent::rectify($order_id,0,'取消确认调整单');
	                parent::operating(__ACTION__,0,'取消确认成功：'.$id);
	                R('Public/errjson',array('ok'));
	            }else {
	                parent::operating(__ACTION__,1,'取消确认失败：'.$rectify_goods->getError());
	                R('Public/errjson',array($rectify_goods->getError()));
	            }
	        }
	    }else {
	...
	public function rectify() {
	    ...
	    if ($this->isAjax()) {
	        ...
	        if ($order_id=='' || !is_numeric($order_id)) {
	            ...
	            $data['order_status'] = 2;
	            if ($rectify->where($where)->save($data)) {
	                D('Warehouse_inventory')->rectify($order_id);
	                parent::rectify($order_id,2,'调整单调整');
	                parent::operating(__ACTION__,0,'调整成功：'.$id);
	                R('Public/errjson',array('ok'));
	            }else {
	                parent::operating(__ACTION__,1,'调整失败：'.$rectify_goods->getError());
	                R('Public/errjson',array($rectify_goods->getError()));
	            }
	        }
	    }else {
	...
	public function complete() {
	    ...
	    if ($this->isAjax()) {
	        ...
	        if ($order_id=='' || !is_numeric($order_id)) {
	            ...
	            $data['order_status'] = 3;
	            if ($rectify->where($where)->save($data)) {
	                parent::rectify($order_id,3,'调整单完成');
	                parent::operating(__ACTION__,0,'完成成功：'.$id);
	                R('Public/errjson',array('ok'));
	            }else {
	                parent::operating(__ACTION__,1,'完成失败：'.$rectify_goods->getError());
	                R('Public/errjson',array($rectify_goods->getError()));
	            }
	        }
	    }else {
