...
<title>商品管理|<?php echo ($configcache['Title']); ?></title>
...
function wincontactEdit(id) {
	if (id=='' || isNaN(id)) {
		...
		fpopload('修改联系人信息',860,430,'__APP__/Goods/contactedit/id/'+id);
		addDiv($('#f_pop'));
	}	
}
$(document).ready(function() {
    $('#content h2 .add').click(function() {
		popload('新增商品',860,500,'__APP__/Goods/Goodsadd/');
		addDiv($('#iframe_pop'));
		popclose();
	});
    $('#content h2 .import').click(function() {
		popload('导入客户',860,500,'__APP__/Goods/Goodsimport/');
		addDiv($('#iframe_pop'));
		popclose();
	});
	function Operating() {
		...
		$('#content #table .openshare').click(function() {
			...
			$.get('__APP__/Goods/opensharedo?id='+id+'&openshare='+opens,function(data) {
				data = eval('('+data+')');
				if (data.s=='ok') {
					if (opens==0) {
		...
		$('#content #table .tr .edit').click(function(event) {
			...
			if (id=='' || isNaN(id)) {
				...
				popload('修改商品信息',860,500,'__APP__/Goods/Goodsedit/id/'+id);
				addDiv($('#iframe_pop'));
				popclose();
			}
		});
	...
	//拉取客户信息
	function Goodsajax(keyword) {
		$.get('__APP__/Goods/indexajax?keyword='+keyword, function(data) {
			//回调函数
			data = eval('('+data+')');
			if (data.s=='ok') {
				//有数据的情况下
...
<div id="content">
    ...
    <div id="page">
	    ...
	    &nbsp;&nbsp;对选中项进行&nbsp;&nbsp;<a href="javascript:;" id="dely">删除</a>
	    &nbsp;&nbsp;&nbsp;&nbsp;
    </div>
</div>
</body>
